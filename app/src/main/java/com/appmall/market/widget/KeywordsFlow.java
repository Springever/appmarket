...
public class KeywordsFlow extends FrameLayout implements OnGlobalLayoutListener{
    ...
    private OnClickListener innerlickListener = new OnClickListener() {
		@Override
		public void onClick(View v) {
			if (mItemClickedListener != null && v instanceof TextView) {
				TextView textView = (TextView) v;
				LabelTag tag = (LabelTag) textView.getTag();
				mItemClickedListener.onItemClicked(tag.mKeyword.mWord, tag.mKeyword.mDataSource);
			}
		}
	};

	private onItemClickListener mItemClickedListener = null;
   ...
   /* public boolean feedKeyword(String keyword) {  
    ...
    public List<Keyword> getKeyTags() {
		return keyTags;
	}

	public void setKeyTags(List<Keyword> keyTags) {
	...
	public int getColor(int index){
		...
		int indentify = getResources().getIdentifier(packageName + ":color/c"+index, null, null);
		return getResources().getColor(indentify);
	}
    ...
    public boolean randomShow(){
    	...
    	if(rd%2==0){
    		return go2Show(ANIMATION_IN);
    	}else{
    		return go2Show(ANIMATION_OUT);
    	}
    }
    ...
    private boolean show() {  
       // if (width > 0 && height > 0 && vecKeywords != null && vecKeywords.size() > 0 && enableShow) {
    	 if (width > 0 && height > 0 && keyTags != null && keyTags.size() > 0 && enableShow) {
            ...
            for (int i = 0; i < size; i++) {  
            	Keyword tag = keyTags.get(i);
                String keyword = tag.mWord.length() > LENGTH ?
                		tag.mWord.substring(0,LENGTH) 
                		: tag.mWord;  
                // 随机颜色  
                //int ranColor = 0xff000000 | random.nextInt(0x0077ffff);
                //int color = random.nextInt(10);
                int ranColor = getColor(random.nextInt(12));
                // 随机位置，糙值  
                ...
                txt.setOnClickListener(innerlickListener);  
                txt.setText(keyword);  
...
//            int[] iXY = (int[]) txt.getTag();
            LabelTag tag = (LabelTag) txt.getTag();
            int[] iXY = tag.mXY;
            ...
            inner: for (int k = i - 1; k >= 0; k--) {  
            	LabelTag labelTag = (LabelTag) txt.getTag();
//                int[] kXY = (int[]) listTxt.get(k).getTag();
            	int[] kXY = labelTag.mXY;
    ...
    private void sortXYList(LinkedList<TextView> listTxt, int endIdx) {  
        for (int i = 0; i < endIdx; i++) {  
            for (int k = i + 1; k < endIdx; k++) {  
            	LabelTag tagK = (LabelTag) listTxt.get(k).getTag();
            	LabelTag tagI = (LabelTag) listTxt.get(i).getTag();
            	
                if (tagK.mXY[IDX_DIS_Y] < tagI.mXY[IDX_DIS_Y]) {  
